...
export default class OrderScreen extends React.Component {
    ...
    async claimOrder() {
        var result = await updateOrderState(
            this.props.apiToken,
            this.props.orderId,
            "claimed"
        );
    ...
    async updateState(state, updateState = true) {
        ...
        if (!result.success) {
            ...
            if (updateState) this.fetchData();
        }
    }

    render() {
        if (this.state.order == null) return <span>Loading</span>;
        ...
        if (this.state.order.state === "unclaimed") {
            ...
            button = (
                <button
                    className="checkoutButton"
                    onClick={() => this.props.openPayment(this.props.orderId)}>
                    Ready To Pay
                </button>
            );
            ...
            button = (
                <button
                    className="checkoutButton"
                    onClick={() => this.updateState("arrived")}>
                    Arrived
                </button>
            );
                ...
                <div className="cartFooter">
                    ...
                    <div className="total">
                        ...
                        <span className="price">
                            ${formatPrice(this.state.order.total)}
                        </span>
                    </div>
                    <div className="buttonsFlex">
                        {button}
                        <div
                            className="messageButtonFlex"
                            onClick={() =>
                                this.props.onMessageClick(this.props.orderId)
                            }></div>
                    </div>
                </div>
            </div>
