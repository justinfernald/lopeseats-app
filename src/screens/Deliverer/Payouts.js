...
export default class Payouts extends React.Component {
    ...
    async fetchData() {
        ...
        for (var i = 0; i < orders.length; i++) {
            var order = orders[i];
            var payoutStatus = await getPayoutStatus(
                this.props.apiToken,
                order.id
            );
                ...
                <div className="incomingOrderList">
                    {this.state.orders == null
                        ...
                        : this.state.orders.map((value, index) => {
                              var color =
                                  value.payoutReceived == 1 ? "#0a0" : "#f00";
                              return (
                                  <div key={index} className="incomingOrder">
                                      <div>
                                          <div
                                              ...
                                              style={{ color: color }}>
                                              Pay out{" "}
                                              {value.payoutReceived == 1
                                                  ? value.status
                                                  : "not received"}
                                              <br />$
                                              {formatPrice(
                                                  value.delivery_fee / 2
                                              )}
                                          </div>
                                      </div>
                ...
                <div className="cartFooter">
                    ...
                    <div className="total">
                        ...
                        <span className="price">
                            ${formatPrice(this.state.payoutTotal)}
                        </span>
                    </div>
                    <button
                        onClick={() => {
                            if (this.state.payoutTotal > 0)
                                this.sendPayoutRequest();
                        }}
                        className={
                            "checkoutButton" +
                            (this.state.payoutTotal == 0 ? " disabled" : "")
                        }>
                        Request Payout
                    </button>
                </div>
