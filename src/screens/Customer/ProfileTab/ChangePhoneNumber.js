...
class ChangePhoneNumber extends React.Component {
    ...
    async save() {
        if (this.currPhoneRef.current.value != this.props.userDetails.phoneNumber) {
            showErrors(['Current phone number is incorrect']);
            this.setState({errors: [true, false]});
            return;
        }

        var success = await store.dispatch(changePhoneNumber(this.newPhoneRef.current.value));
...
export default connect(({userDetails}) => ({userDetails}))(ChangePhoneNumber);
